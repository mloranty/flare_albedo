...
# create var with latitudinal bins
fr <- fr %>% mutate(lat.bin = cut(lat.r, breaks = c(49,60,65,77)))
...
# group by these bins
f.lat <- fr %>%
  group_by(yr.bin,lat.bin) %>%
  summarise(area = sum(SizeHa), severity = mean(dnbr))
...
f.lat1 <- fr %>%
  group_by(as.factor(lat.r)) %>%
  summarise(area = sum(SizeHa), sv = weighted.mean(dnbr, SizeHa), sv.sd = sd(dnbr))
f.lat1$lat <- as.numeric(f.lat1$`as.factor(lat.r)`)

# create and print a barplot of area burned by latitude
bp <- ggplot(f.lat, aes(fill=yr.bin, y=area/10^6, x=lat.bin)) + 
      geom_col(position="dodge") +
      xlab("Latitude") + ylab("Area Burned (Millions of Hectares)") +
      scale_fill_manual(values = c('#a6cee3','#1f78b4','#b2df8a','#33a02c'),
                        labels = c("2001-2005","2006-2010","2011-2015","2016-2020")) +
      scale_x_discrete(labels = c("50-60", "60-65", "65 <")) +
     # scale_fill_discrete(c("2001-2005","2006-2010","2011-2015","2016-2020")) +
      labs(fill = "") +theme_bw()

ggsave("figures/area_burned_by_latitude_uneven_bins.png",
...
ggsave("figures/canopy_cover_by_latitude.png",
       width = 6, height = 4, units = "in")
# monthly mean albedo by ecozone, biome, and region
# note - this plot is pretty messy as there is a lot of interannual variability
###################################################################
# get unique ecozone from fire data set
eco <- unique(fire$EcoCode)

# get fire ids for first ecozone
f <- fire$UniqueId[which(fire$EcoCode == eco[1])]

# create data frame with first ecozone
alb.eco <- as.data.frame(cbind(alb.tsf$ysf, alb.tsf$month, alb.tsf$day,
              rowMeans(alb.tsf[,na.omit(match(f,colnames(alb.tsf)))],na.rm = T)))

# add remaining ecozones
for( i in 2:length(eco))
{
  f <- fire$UniqueId[which(fire$EcoCode == eco[i])]
  alb.eco <- cbind(alb.eco,rowMeans(alb.tsf[,na.omit(match(f,colnames(alb.tsf)))],na.rm = T))
}

# add arctic and subarctic
f <- fire$UniqueId[which(fire$ArcSub == "subarctic")]
alb.eco <- cbind(alb.eco,rowMeans(alb.tsf[,na.omit(match(f,colnames(alb.tsf)))],na.rm = T))
f <- fire$UniqueId[which(fire$ArcSub == "arctic")]
alb.eco <- cbind(alb.eco,rowMeans(alb.tsf[,na.omit(match(f,colnames(alb.tsf)))],na.rm = T))

#add biome
f <- fire$UniqueId[which(fire$biome == "Boreal")]
alb.eco <- cbind(alb.eco,rowMeans(alb.tsf[,na.omit(match(f,colnames(alb.tsf)))],na.rm = T))
f <- fire$UniqueId[which(fire$biome == "Tundra")]
alb.eco <- cbind(alb.eco,rowMeans(alb.tsf[,na.omit(match(f,colnames(alb.tsf)))],na.rm = T))
# set column names 
names(alb.eco) <- c("ysf", "month", "day",eco,"subarctic", "arctic","boreal","tundra")

# aggregate to monthly
alb.eco.m <- aggregate(alb.eco[,4:15], by = list(alb.eco$ysf, alb.eco$month), mean)

# set column names 
names(alb.eco.m) <- c("ysf", "month",eco,"subarctic", "arctic","boreal","tundra")

# create data frame with biome/region
alb.reg.m <- alb.eco.m[,c(1:2,11:14)]

# drop biome/region from ecozone 
alb.eco.m <- alb.eco.m[,1:10]

# plot albedo as a function of year since fire by month

# pivot data into long format
aem <- pivot_longer(alb.eco.m,
                    cols = 3:10,
                    names_to = "eco",
                    values_to = "albedo")

arm <- pivot_longer(alb.reg.m,
                    cols = 3:6,
                    names_to = "reg",
                    values_to = "albedo")

# create a multipanel plot for ecozones
p <- ggplot(data = aem, aes(x = ysf, y = albedo)) + 
      geom_vline(xintercept = 0, linetype = "dashed",
             color = "darkgray", linewidth = 1) +
      geom_point((aes(color = eco)), na.rm = T) + 
      labs(color = "Ecozone") +
  #scale_color_brewer("Dark2") +
  theme_bw()

p + facet_wrap(vars(month), scales = "free_y",
               labeller = as_labeller(c("3" = "March",
                                        "4" = "April",
                                        "5" = "May",
                                        "6" = "June",
                                        "7" = "July",
                                        "8" = "August",
                                        "9" = "September"))) +
  xlab("Year Since Fire") + ylab("Albedo")

ggsave("ecozone_albedo_weekly_by_month.png",
       width = 6, height = 4, units = "in")

# create a multipanel plot for regions
p <- ggplot(data = arm, aes(x = ysf, y = albedo)) + 
  geom_vline(xintercept = 0, linetype = "dashed",
             color = "darkgray", size = 1) +
  geom_point((aes(color = reg)), na.rm = T) + 
  labs(color = "Ecozone") +
  #scale_color_brewer("Dark2") +
  theme_bw() 
...
# save plot to file
ggsave("figures/region_albedo_weekly_by_month.png",
       width = 6, height = 4, units = "in")
###################################################################
# plot pre-fire annual albedo trajectories (averaged for all pre-fire years)
# first by ecozone, and then by region/biome
###################################################################
# get unique ecozone from fire data set
eco <- unique(fire$EcoCode)
...
# create data frame with first ecozone
pre.eco <- as.data.frame(cbind(pre$month, pre$day,
...
for( i in 2:length(eco))
{
  ...
  pre.eco <- cbind(pre.eco,rowMeans(pre[,na.omit(match(fid,colnames(pre)))],na.rm = T))
}

# set column names
names(pre.eco) <- c("month", "day",eco)

# add julian day for plotting (assume non-leap year)
pre.eco$jday <- yday(strptime(paste(pre.eco$month,pre.eco$day,"2010", sep = "-"), 
                              format = "%m-%e-%Y", tz = ""))
# LATITUDE BY 5 DEGREE BINS
...
# CREATE SEPARATE DATA FRAME FOR BIOME/REGION     
pre.reg <- as.data.frame(cbind(pre.eco$month, pre.eco$day, pre.eco$jday))

# add arctic and subarctic
fid <- fire$UniqueId[which(fire$ArcSub == "subarctic")]
pre.reg <- cbind(pre.reg,rowMeans(pre[,na.omit(match(fid,colnames(pre)))],na.rm = T))
...
pre.reg <- cbind(pre.reg,rowMeans(pre[,na.omit(match(fid,colnames(pre)))],na.rm = T))
# set column names 
names(pre.reg) <- c("month", "day","jday","Subarctic", "Arctic","Boreal","Tundra")

# DATA TO LONG FORMAT FOR PLOTTING AND ANALYSIS
pea <- pivot_longer(pre.eco,
                    cols = 3:10,
                    names_to = "eco",
                    values_to = "albedo")

pla <- pivot_longer(pre.lat,
                    cols = 3:7,
                    names_to = "lat",
                    values_to = "albedo")

pra <- pivot_longer(pre.reg,
                    cols = 4:7,
                    names_to = "reg",
                    values_to = "albedo")
...
###################################################################
# plot prefire albedo and latitudinal fire characteristics
###################################################################
# create a plot for regions
p1 <- ggplot(data = pra, aes(x = jday, y = albedo)) + 
  geom_line((aes(color = reg)), na.rm = T, linewidth = 1.5) + 
  labs(color = "Region") +
  xlab("Julian Day") +
  ylab("Albedo") +
  #scale_fill_discrete(breaks=c("Arctic","Subarctic","Boreal","Tundra")) +
  scale_color_manual(values = c("lightblue", "blue", "lightgreen", "darkgreen"),
                    breaks=c("Arctic","Subarctic","Tundra","Boreal")) +
  #scale_color_brewer("Dark2") +
  theme_bw(base_size = 16) +
...
p2 <- ggplot(data = pea, aes(x = jday, y = albedo)) + 
  geom_line((aes(color = eco)), na.rm = T, linewidth = 1.5) + 
  labs(color = "Ecozone") +
  xlab("Julian Day") +
  ylab("Albedo") +
  #scale_color_brewer("Dark2") +
...
p3 <- ggplot(data = pla, aes(x = jday, y = albedo, col=lat)) + 
  geom_line(na.rm = T, linewidth = 1.5) + 
  #scale_color_viridis_d(
  scale_color_manual(values = hcl.colors(8,palette = "Blues 3",rev=T)[4:8],
  #scale_color_brewer(palette="Spectral",
                     labels = c("70 <","66-70","61-65","56-60","50-55"),
                     breaks = c("(70,77]","(65,70]","(60,65]","(55,60]","(49,55]")) +
  labs(color = "Latitude", x = "Julian Day", y = "Albedo") +
  theme_bw(base_size = 16) +
...
# save plot to file
ggsave("figures/prefire_seasonal_albedo.png",
...
# save plot to file
ggsave("figures/prefire_seasonal_albedo_cpy.png",
       width = 12, height = 4, units = "in")


#two panel plot
cc+theme_bw(base_size = 18) +p3+theme_bw(base_size = 18) 

# save plot to file
ggsave("figures/prefire_seasonal_albedo_cpy_lat.png",
...
a + facet_wrap(vars(lat), scales = "free_y",
              labeller = as_labeller(c("(49,55]" = "50-55",
                         "(55,60]" = "55-60",
                         "(60,65]" = "60-65",
                         "(65,70]" = "65-70",
                         "(70,77]" = "70 < "))) +
